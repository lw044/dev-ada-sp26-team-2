...
// ── Analysis flow (direct fetch, no background script) ───────────────────────
async function kickOff(text) {
  killFloatBtn();
  showOverlay("loading", null, text);
 
  try {
    const res = await fetch(`${API_URL}?text=${encodeURIComponent(text)}`, {
      method: "POST",
    ...
    // predict_web returns a plain string e.g. "happiness"
    const label = (typeof raw === "string" ? raw : raw.tone ?? raw.label ?? "neutral").toLowerCase();
    const toneId = TONE_MAP[label] ?? 6;
    showOverlay("result", toneId, text);
 
  } catch (err) {
    const msg = err.name === "TimeoutError"
      ? "Timed out — backend may be cold-starting, try again in 30s"
      : err.message;
    showOverlay("error", null, text, msg);
  }
}
